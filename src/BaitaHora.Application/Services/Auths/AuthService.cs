...
namespace BaitaHora.Application.Services.Auths
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<Result<UserResponse>> RegisterOwnerWithCompanyAsync(RegisteOwnerRequest request)
        {
            ...
            if (!string.IsNullOrWhiteSpace(request.Company.Document) &&
                await _companyRepository.ExistsByDocumentAsync(request.Company.Document))
                return Result<UserResponse>.Failure("Documento da empresa já está em uso.");

            if (await _companyRepository.ExistsByNameAsync(request.Company.Name))
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(request.Company.ImageUrl))
                {
                    var image = new CompanyImage(company.Id, request.Company.ImageUrl!.Trim());
                    await _companyRepository.AddImageAsync(image);
                }

                var member = new CompanyMember(company.Id, user.Id, CompanyRole.Owner, DateTime.UtcNow, true);
            ...
            {
                _logger.LogWarning(ex, "Erro de domínio ao bootstrap de empresa/owner {Email}", request.User.Email);
                await _uow.RollbackAsync();
                return Result<UserResponse>.Failure(ex.Message);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Erro inesperado ao bootstrap de empresa/owner {Email}", request.User.Email);
                await _uow.RollbackAsync();
